...
def generate_launch_description():
    ...
    output_dir_arg = DeclareLaunchArgument(
        'output_dir',
        default_value='/home/hu/usv_captures/pseudocolor_single',
        description='Directory for pseudo-color gated annotation images'
    )
    ...
    occlusion_mode_arg = DeclareLaunchArgument(
        'occlusion_mode',
        default_value='false',
        description='Use the complex multi-object occlusion annotation scene'
    )
    ...
    sim = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'c3_mmwave': 'false',
            'rgbd_dehaze': 'false',
            'stf_gated_fusion': 'false',
            'gated_bev_detection': 'false',
    ...
    dynamic_targets = Node(
        ...
        parameters=[
            perception_config,
            {
                ...
                'annotation_occlusion_mode': ParameterValue(LaunchConfiguration('occlusion_mode'), value_type=bool),
    ...
    recorder = Node(
        ...
        name='pseudocolor_image_recorder',
        output='screen',
        parameters=[{
            'use_sim_time': True,
            'image_topic': '/gated_camera/range_view',
        ...
        condition=IfCondition(PythonExpression(["'", LaunchConfiguration('max_images'), "' != '0'"]))
    )

    shutdown_when_done = RegisterEventHandler(
        OnProcessExit(
            target_action=recorder,
            on_exit=[EmitEvent(event=Shutdown(reason='pseudo-color annotation image capture finished'))],
        )
    )
