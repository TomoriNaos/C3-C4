...
{
    const auto rgb_topic = declare_parameter<std::string>("rgb_topic", "/camera/image_raw");
    ...
    const auto pointcloud_topic = declare_parameter<std::string>("pointcloud_topic", "pointcloud");
    ...
    show_images_ = declare_parameter<bool>("show_images", false);
    ...
    pointcloud_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>(pointcloud_topic, 10);

    RCLCPP_INFO(
        get_logger(),
        "RGB-D dehaze subscribed to rgb=%s depth=%s camera_info=%s, publishing pointcloud=%s",
        rgb_topic.c_str(),
        depth_topic.c_str(),
