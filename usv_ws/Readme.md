...
下面命令默认你站在 `project_root`，也就是同时能看到 `usv_ws/` 和 `yolo/` 的目录：
...
如果你已经站在 `usv_ws` 目录里，例如终端提示符是 `~/usv_ws$`，则使用：

```bash
export USV_WS=$PWD
export YOLO_DIR=$HOME/yolo
```

也可以直接写死为本机路径：

```bash
export USV_WS=$HOME/usv_ws
export YOLO_DIR=$HOME/yolo
```
...
如果提示 `./scripts/build_clean_env.sh: 没有那个文件或目录`，先检查当前路径是否真的是工程根目录：
...
正确的 `pwd` 应类似 `/home/hu/usv_ws`，并且能够看到 `src/`、`scripts/`、`Readme.md`。如果你已经在 `/home/hu/usv_ws`，但之前设置过 `export USV_WS=$PWD/usv_ws`，会误指向 `/home/hu/usv_ws/usv_ws`，请改用：
...
脚本会重新调用 `best.onnx` 和 `best1.onnx` 做验证集推理，并在 `eval_outputs/` 根目录直接重绘展示图。若当前没有 `sonar_capture.json`，脚本会使用固定参考数据生成说明书/PPT 结构图和消融图；正式实验结论仍以评估脚本、rosbag 或实际 JSON 统计为准。
...
这些图片用于复现展示样例、系统框架、消息对齐、热力图和消融效果。完整六类准确率仍以 `summary.json`、上表和原始实验数据为准。
...
`ais_simulator` 从 Gazebo `/model_states` 中读取目标船、渔船、远处大船等模型的位置和速度，转换到无人船 `base_link` 相对坐标后发布 JSON。每个 AIS 目标包含：

```text
mmsi        仿真 MMSI，从 413000100 开始递增
name        Gazebo 目标模型名
x/y         相对无人船位置，单位 m
vx/vy       相对无人船速度，单位 m/s
sog         对地航速
cog         对地航向
heading     船体艏向
confidence  AIS 先验置信度
class_id    fishing_boat 为 2，其余目标船为 5
```

当前 AIS 主要接入传统 `radar_sonar_tracker`，作用是提供远距离目标船先验、稳定 track ID 和辅助目标选择；C3 热力图主路径仍以毫米波雷达、声呐、门控/普通/BEV 融合模态和深度相机点云为主。检查 AIS 是否工作：

```bash
ros2 topic echo /ais/targets --once
