...
def draw_example(
    ...
    def display_name(index: int) -> str:
        if index < 0 or index >= len(names):
            return str(index)
        return DISPLAY_NAMES.get(names[index], names[index])

    canvas = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
    draw = ImageDraw.Draw(canvas)
    text_font = label_font(18)
    for i, gt in enumerate(gts):
        color = (0, 210, 0) if i in matched_gt else (255, 0, 0)
        x1, y1, x2, y2 = [int(v) for v in gt.xyxy]
        draw.rectangle((x1, y1, x2, y2), outline=color, width=3)
        draw.text((x1, max(2, y1 - 24)), f"真值 {display_name(gt.cls)}", fill=color, font=text_font)
    for i, pred in enumerate(preds):
        color = (255, 210, 0) if i in matched_pred else (180, 65, 220)
        x1, y1, x2, y2 = [int(v) for v in pred.xyxy]
        draw.rectangle((x1, y1, x2, y2), outline=color, width=3)
        draw.text((x1, min(canvas.height - 24, y2 + 4)), f"预测 {display_name(pred.cls)} {pred.score:.2f}", fill=color, font=text_font)
