...
namespace c3_drone_driver
{
	...
	private:
		...
		void publishIfReady()
		{
			...
			obs.source = source;
			obs.status = (obs.confidence >= low_conf_threshold_) ? msg::TargetObservation::STATUS_VALID : msg::TargetObservation::STATUS_LOW_CONF;
			...
			mavlink_observation_pub_->publish(obs);
